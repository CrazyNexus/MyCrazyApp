//
// ShippersMasterTableDelegate.swift
// MyCrazyApp
//
...
class ShippersMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [Shipper] = [Shipper]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [Shipper]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadShippers { (shippers, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let shipper = self.entities[indexPath.row] as! Shipper
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ShipperID", value: "\(shipper.shipperID)")
        return cell
    }
