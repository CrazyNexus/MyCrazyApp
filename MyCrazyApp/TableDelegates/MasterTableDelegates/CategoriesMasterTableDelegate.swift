//
// CategoriesMasterTableDelegate.swift
// MyCrazyApp
//
...
class CategoriesMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [Category] = [Category]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [Category]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadCategories { (categories, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let category = self.entities[indexPath.row] as! Category
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CategoryID", value: "\(category.categoryID)")
        return cell
    }
