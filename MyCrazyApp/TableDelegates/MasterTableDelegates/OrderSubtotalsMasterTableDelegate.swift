//
// OrderSubtotalsMasterTableDelegate.swift
// MyCrazyApp
//
...
class OrderSubtotalsMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [OrderSubtotal] = [OrderSubtotal]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [OrderSubtotal]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadOrderSubtotals { (ordersubtotals, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let ordersubtotal = self.entities[indexPath.row] as! OrderSubtotal
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(ordersubtotal.orderID)")
        return cell
    }
