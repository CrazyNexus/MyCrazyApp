//
// SummaryOfSalesByYearsMasterTableDelegate.swift
// MyCrazyApp
//
...
class SummaryOfSalesByYearsMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [SummaryOfSalesByYear] = [SummaryOfSalesByYear]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [SummaryOfSalesByYear]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadSummaryOfSalesByYears { (summaryofsalesbyyears, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let summaryofsalesbyyear = self.entities[indexPath.row] as! SummaryOfSalesByYear
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(summaryofsalesbyyear.orderID)")
        return cell
    }
