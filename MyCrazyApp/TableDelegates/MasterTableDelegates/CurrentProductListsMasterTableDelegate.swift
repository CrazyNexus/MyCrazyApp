//
// CurrentProductListsMasterTableDelegate.swift
// MyCrazyApp
//
...
class CurrentProductListsMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [CurrentProductList] = [CurrentProductList]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [CurrentProductList]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadCurrentProductLists { (currentproductlists, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let currentproductlist = self.entities[indexPath.row] as! CurrentProductList
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ProductID", value: "\(currentproductlist.productID)")
        return cell
    }
