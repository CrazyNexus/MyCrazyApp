//
// InvoicesMasterTableDelegate.swift
// MyCrazyApp
//
...
class InvoicesMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [Invoice] = [Invoice]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [Invoice]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadInvoices { (invoices, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let invoice = self.entities[indexPath.row] as! Invoice
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CustomerName", value: "\(invoice.customerName)")
        return cell
    }
