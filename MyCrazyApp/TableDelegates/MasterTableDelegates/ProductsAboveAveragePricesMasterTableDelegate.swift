//
// ProductsAboveAveragePricesMasterTableDelegate.swift
// MyCrazyApp
//
...
class ProductsAboveAveragePricesMasterTableDelegate: NSObject, MasterTableDelegate {
    ...
    private var _entities: [ProductsAboveAveragePrice] = [ProductsAboveAveragePrice]()
    var entities: [EntityValue] {
        get { return _entities }
        set { self._entities = newValue as! [ProductsAboveAveragePrice]
        }
    }
    ...
    func requestEntities(completionHandler: @escaping(Error?) -> Void) {
        self.dataAccess.loadProductsAboveAveragePrices { (productsaboveaverageprices, error) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let productsaboveaverageprice = self.entities[indexPath.row] as! ProductsAboveAveragePrice
        let cell = cellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ProductName", value: "\(productsaboveaverageprice.productName)")
        return cell
    }
